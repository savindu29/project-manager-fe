...
interface MyFileInputProps {
  id: string;
  onSelectFiles: (id: string, files: { file: File; description: string }[]) => void;
  isDisabled: boolean;
}
  ...
  return (
    <div className="mt-4">
      ...
      <div className="flex items-center space-x-4">
        <label htmlFor={`fileInput-${id}`} className="flex items-center justify-center w-40 h-10 bg-gray-200 text-gray-600 rounded-md cursor-pointer  focus-within:outline-none focus-within:ring focus-within:border-sky-300">
          <input
            id={`fileInput-${id}`}
            type="file"
            className="sr-only"
        ...
        <div className="mt-4">
          ...
          <ul className="grid gap-2 grid-cols-2">
            {selectedFiles.map((file, index) => (
              <li key={index} className="flex flex-col p-2 border border-gray-300 rounded-md bg-gray-100">
                <span className="truncate mb-2" onClick={() => openDescriptionModal(index)}>
                  {`${file.description || 'No description'} - ${file.file.type}`}
                </span>
                {/* <button
                  type="button"
      ...
      {currentFileIndex !== null && (
        <div className={`fixed inset-0 z-50 flex items-center justify-center  ${showDescriptionModal ? '' : 'hidden'}`}>
          <div className="fixed inset-0 bg-black opacity-50"></div>
          <div className="bg-white p-4 rounded-md z-10 w-96">
            <label className="block text-sm font-medium text-gray-700 mb-2">Rename</label>
            <input
              type="text"
